...
function TemplateCard({ tpl, onEdit, onDelete, onDuplicate }: {
    ...
    return (
        <div className="p-5 rounded-2xl border border-white/10 bg-white/5 hover:bg-white/8 transition-all flex flex-col gap-3">
            <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-600">{tpl.use_count} use{tpl.use_count !== 1 ? "s" : ""}</p>
                    </div>
                </div>
                <div className="flex items-center gap-0.5">
                    <button onClick={() => onDuplicate(tpl)}
                        className="p-1.5 rounded-lg text-slate-500 hover:text-slate-300 hover:bg-white/10 transition-colors" title="Duplicate">
                        <Copy size={13} />
                    </button>
                    <button onClick={() => onEdit(tpl)}
                        className="p-1.5 rounded-lg text-slate-500 hover:text-slate-300 hover:bg-white/10 transition-colors" title="Edit">
                        <Edit2 size={13} />
                    </button>
            ...
            <div className="flex items-center gap-2 flex-wrap">
                {tpl.priority && (
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold capitalize ${PRIORITY_BADGE[tpl.priority] || "bg-slate-500/20 text-slate-400"}`}>
                        {tpl.priority}
                    </span>
                )}
                {tpl.task_type && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-indigo-500/15 text-indigo-400">
                        #{tpl.task_type}
                    </span>
                )}
                {tpl.estimated_hours && (
                    <span className="text-[10px] text-slate-600">{tpl.estimated_hours}h estimated</span>
                )}
            </div>
                ...
                <div className="space-y-1">
                    {tpl.checklist.slice(0, 3).map((item, i) => (
                        <div key={i} className="flex items-center gap-2 text-xs text-slate-500">
                            <CheckCircle2 size={10} className="text-slate-600 shrink-0" />
                            <span className="truncate">{item}</span>
                        </div>
                    ))}
                    {tpl.checklist.length > 3 && (
                        <p className="text-[10px] text-slate-700">+{tpl.checklist.length - 3} more…</p>
                    )}
                </div>
            )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col">
                <div className="flex items-center justify-between p-6 border-b border-white/10">
                    <h2 className="text-lg font-semibold text-slate-200">{tpl ? "Edit Template" : "New Template"}</h2>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="block text-sm text-slate-400 mb-1">Default Status</label>
                    ...
                    {/* Checklist */}
                    <div>
                        <label className="block text-sm text-slate-400 mb-1">Checklist Items</label>
                        <div className="flex gap-2 mb-2">
                            ...
                            <button onClick={addChecklistItem} className="px-3 py-2 rounded-lg bg-white/10 text-slate-300 hover:bg-white/15 transition-colors text-sm">+</button>
                        </div>
                        {form.checklist.map((item, i) => (
                            <div key={i} className="flex items-center gap-2 mb-1">
                                <CheckCircle2 size={12} className="text-slate-600 shrink-0" />
                                <span className="flex-1 text-sm text-slate-400">{item}</span>
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 mb-2">
                            ...
                            <button onClick={addTag} className="px-3 py-2 rounded-lg bg-white/10 text-slate-300 hover:bg-white/15 transition-colors text-sm">+</button>
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                            {form.tags.map((tag) => (
                                <span key={tag} className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-indigo-500/15 text-indigo-400 text-xs">
                                    {tag}
    ...
    return (
        <div className="min-h-screen p-6 space-y-6 bg-background text-foreground">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    <h1 className="text-2xl font-bold text-slate-100 flex items-center gap-2">
                        <FileText size={22} className="text-indigo-400" /> Task Templates
                    </h1>
                    <p className="text-sm text-slate-500 mt-1">{templates.length} templates · reusable task blueprints</p>
                </div>
            ...
            <input
                ...
                className="w-full max-w-sm px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-300 placeholder-slate-600 focus:outline-none focus:border-indigo-500/50 text-sm"
            />
            ...
            {deleteTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
                        <h3 className="text-base font-semibold text-slate-200 mb-2">Delete Template</h3>
                        <p className="text-sm text-slate-400 mb-5">Delete "{deleteTarget.name}"? This cannot be undone.</p>
