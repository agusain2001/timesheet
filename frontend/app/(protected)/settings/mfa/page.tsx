...
export default function MFAPage() {
    ...
    const inputCls = "w-full px-4 py-3 rounded-xl bg-foreground/[0.02] border border-foreground/10 text-foreground/90 focus:outline-none focus:border-blue-500/50 text-base tracking-[0.3em] text-center";
            ...
            <div className="max-w-lg mx-auto space-y-6">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
                        <Shield size={22} className="text-blue-400" /> Two-Factor Authentication
                    </h1>
                    <p className="text-sm text-foreground/50 mt-1">Secure your account with TOTP-based two-factor authentication</p>
                </div>

                {/* Status Card */}
                <div className={`p-5 rounded-2xl border flex items-center gap-4 ${isEnabled ? "border-green-500/30 bg-green-500/5" : "border-foreground/10 bg-foreground/[0.01]"
                    }`}>
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isEnabled ? "bg-green-500/20" : "bg-slate-500/20"
                        }`}>
                        {isEnabled ? <ShieldCheck size={22} className="text-green-400" /> : <ShieldOff size={22} className="text-foreground/50" />}
                    </div>
                    <div className="flex-1">
                        <p className="font-semibold text-foreground/90">{isEnabled ? "MFA is Enabled" : "MFA is Disabled"}</p>
                    ...
                    <div className={`px-2.5 py-1 rounded-full text-xs font-semibold ${isEnabled ? "bg-green-500/20 text-green-400" : "bg-slate-700 text-foreground/50"}`}>
                ...
                {step === "status" && (
                    <div className="space-y-4">
                        <div className="rounded-2xl border border-foreground/10 bg-foreground/[0.01] p-5 space-y-3">
                            <h3 className="text-sm font-semibold text-foreground/80">How it works</h3>
                            <div className="space-y-2">
                                {[
                                    ...
                                    <div key={i} className="flex items-start gap-3">
                                        ...
                                        <p className="text-sm text-foreground/60">{s}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <button onClick={startSetup} disabled={processing}
                            className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-medium transition-colors disabled:opacity-50">
                ...
                {step === "setup" && setupData && (
                    <div className="space-y-5">
                        <div className="rounded-2xl border border-foreground/10 bg-foreground/[0.01] p-5 flex flex-col items-center gap-4">
                            <p className="text-sm text-foreground/60 text-center">Scan this QR code with your authenticator app, or enter the secret key manually.</p>
                            ...
                            <div className="w-full">
                                <p className="text-xs text-foreground/50 mb-1">Secret Key (manual entry)</p>
                                <div className="flex items-center gap-2">
                                    <code className="flex-1 px-3 py-2 rounded-lg bg-foreground/[0.02] border border-foreground/10 text-blue-300 text-sm font-mono tracking-wider break-all">
                                        {setupData.secret}
                                    </code>
                                    <button onClick={() => copy(setupData.secret)}
                                        className="p-2 rounded-lg bg-foreground/[0.02] border border-foreground/10 text-foreground/60 hover:text-foreground/90 transition-colors">
                        ...
                        <div>
                            <p className="text-sm text-foreground/60 mb-1">Enter the 6-digit code from your app</p>
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => { setStep("status"); setSetupData(null); setCode(""); }}
                                className="flex-1 py-2.5 rounded-xl bg-foreground/[0.02] border border-foreground/10 text-foreground/60 hover:text-foreground/90 text-sm transition-colors">
                                Cancel
                            </button>
                            <button onClick={verify} disabled={processing || code.length !== 6}
                                className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium transition-colors disabled:opacity-50">
                        ...
                        {setupData.backup_codes.length > 0 && (
                            <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
                                ...
                                <p className="text-xs text-foreground/50 mb-3">Store these securely. Each can be used once if you lose your authenticator.</p>
                                <div className="grid grid-cols-2 gap-1.5">
                                    {setupData.backup_codes.map((c, i) => (
                                        <code key={i} className="px-2 py-1 rounded bg-foreground/[0.02] text-foreground/80 text-xs font-mono">{c}</code>
                                    ))}
                                </div>
                    ...
                    <div className="space-y-4">
                        <div className="rounded-2xl border border-foreground/10 bg-foreground/[0.01] p-5 space-y-3">
                            <div className="flex items-center justify-between">
                                <h3 className="text-sm font-semibold text-foreground/80">Recovery Codes</h3>
                            ...
                            {backupCodes.length > 0 && (
                                <div className="grid grid-cols-2 gap-1.5">
                                    {backupCodes.map((c, i) => (
                                        <code key={i} className="px-2 py-1 rounded bg-foreground/[0.02] text-foreground/60 text-xs font-mono">{c}</code>
                                    ))}
                                </div>
                            )}
                            {backupCodes.length === 0 && (
                                <p className="text-xs text-foreground/40">Click "View codes" to reveal your recovery codes.</p>
                            )}
                        </div>
                        ...
                        {showDisable && (
                            <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-5 space-y-3">
                                <p className="text-sm text-foreground/60">Enter your current authenticator code to disable MFA.</p>
                                ...
                                <div className="flex gap-3">
                                    <button onClick={() => { setShowDisable(false); setDisableCode(""); setError(""); }}
                                        className="flex-1 py-2 rounded-xl bg-foreground/[0.02] border border-foreground/10 text-foreground/60 hover:text-foreground/90 text-sm transition-colors">
                                        Cancel
                                    </button>
