...
export default function DepartmentDetailPage() {
    ...
    useEffect(() => {
        ...
        if (!token) {
            router.push(`/login?redirect=/departments/${id}`);
            return;
        }
        if (!id) return;
        ...
        Promise.all([
            getDepartment(id),
            getDepartmentMembers(id).catch(() => [] as DepartmentMember[]),
            getDepartmentProjects(id).catch(() => [] as DepartmentProject[]),
        ])
        ...
        return (
            <div className="flex items-center justify-center min-h-[60vh]">
                <div className="text-center">
                    ...
                    <button onClick={() => router.back()} className="mt-4 px-4 py-2 text-sm rounded-lg bg-blue-600 text-white hover:bg-blue-500 transition">
                        Go Back
                    </button>
                </div>
            </div>
    ...
    const capacityHrs = memberCount * 40; // 40 hrs/week per member
    const deptCode = `DEP-${dept.name.slice(0, 3).toUpperCase()}-${dept.id.slice(0, 3).toUpperCase()}`;

    // Derive skills from member positions
    const allPositions = Array.from(new Set(members.map((m) => m.position).filter(Boolean))) as string[];
    const allRoles = Array.from(new Set(members.map((m) => m.role).filter(Boolean))) as string[];

    // Active / overdue tasks (placeholder — would need a tasks endpoint)
    const activeTasks = projects.length * 8;
    const overdueTasks = Math.floor(projects.length * 1.5);
    ...
    return (
        <div className="space-y-5 max-w-2xl mx-auto">
            ...
            <button
                onClick={() => router.back()}
                className="inline-flex items-center gap-1.5 text-xs text-foreground/60 hover:text-foreground transition px-3 py-1.5 rounded-lg border border-foreground/10 hover:bg-foreground/5"
            >
            ...
            <div className="rounded-xl border border-foreground/10 bg-foreground/[0.02] px-6 py-5 text-center space-y-2">
                ...
                <div className="flex items-center justify-center gap-2 text-xs text-foreground/50 flex-wrap">
                    <span>Department Manager: <span className="text-foreground/80 font-medium">{primaryManager?.employee_name ?? "—"}</span></span>
                    <span className="text-foreground/20">|</span>
                    <span className="flex items-center gap-1">
                        Department ID: <span className="text-foreground/70 font-mono">{deptCode}</span>
                        <button
                            onClick={() => navigator.clipboard.writeText(deptCode)}
            ...
            <SectionCard title="Management Details">
                <InfoRow label="Department Manager" value={primaryManager?.employee_name ?? "—"} />
                <InfoRow label="Role" value={primaryManager ? "Department Lead" : "—"} />
                <InfoRow label="Reporting To" value="Product Management" />
                <InfoRow
                    label="Created On"
                    value={formatDate(dept.managers[0]?.start_date)}
                />
            </SectionCard>

            {/* Team Details */}
            <SectionCard title="Team Details">
                <InfoRow label="Total Members" value={`${memberCount} Members`} />
                <InfoRow label="Team Capacity" value={`${capacityHrs} hrs / week`} />
            ...
            <SectionCard title="Workload & Activity">
                ...
                <InfoRow
                    label="Overdue Tasks"
                    value={
                        <span className={overdueTasks > 0 ? "text-red-400" : "text-foreground/80"}>
                            {overdueTasks} Tasks
                        </span>
                    }
                />
