    ...
    return (
        <div className={`flex items-start gap-3 p-4 rounded-2xl border transition-all group ${note.is_read
            ...
            <div className="flex-1 min-w-0">
                ...
                {note.link && (
                    <a href={note.link} className="text-xs text-blue-400 hover:text-blue-300 mt-1 block" onClick={(e) => e.stopPropagation()}>
                        View →
                    </a>
                )}
            </div>
    ...
    return (
        <div className="max-w-2xl space-y-4">
            <div className="flex items-center justify-between">
                <p className="text-sm text-foreground/60">Automate when you get notified about task deadlines and escalations.</p>
        ...
        <div className="space-y-6 bg-background text-foreground">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    <h1 className="text-2xl font-bold flex items-center gap-2 text-foreground">
                        <Bell size={22} className="text-blue-400" /> Notifications
                    </h1>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    {tab === "inbox" && unread > 0 && (
                        <button onClick={handleReadAll}
                            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-foreground/[0.02] border border-foreground/10 text-foreground/60 hover:text-foreground/90 text-sm transition-colors">
            ...
            <div className="flex gap-1 bg-foreground/5 border border-foreground/10 rounded-xl p-1 w-fit">
                <button
                    ...
                    className={tab === "inbox" ? "px-4 py-1.5 rounded-lg text-sm font-medium capitalize transition-colors bg-blue-600 text-white" : "px-4 py-1.5 rounded-lg text-sm font-medium capitalize transition-colors text-foreground/50 hover:text-foreground"}
                >
                    {unread > 0 ? `Inbox (${unread})` : "Inbox"}
                </button>
                <button
                    onClick={() => setTab("rules")}
                    className={tab === "rules" ? "px-4 py-1.5 rounded-lg text-sm font-medium capitalize transition-colors bg-blue-600 text-white" : "px-4 py-1.5 rounded-lg text-sm font-medium capitalize transition-colors text-foreground/50 hover:text-foreground"}
                >
                    Rules
                </button>
            </div>
            ...
            {tab === "rules" ? <RulesPanel /> : (
                <>
                    {/* Filters */}
                    <div className="flex items-center gap-2 flex-wrap">
                        <div className="flex bg-foreground/[0.02] border border-foreground/10 rounded-xl p-1 gap-0.5">
                            {(["all", "unread"] as const).map((f) => (
                                <button
                                    key={f}
                                    onClick={() => setFilter(f)}
                                    className={f === filter ? "px-3 py-1.5 rounded-lg text-xs font-medium transition-colors capitalize bg-blue-600 text-white" : "px-3 py-1.5 rounded-lg text-xs font-medium transition-colors capitalize text-foreground/50 hover:text-foreground/80"}
                                >
                        ...
                        {uniqueTypes.length > 1 && (
                            <select
                                ...
                                className="px-3 py-2 rounded-xl bg-foreground/[0.02] border border-foreground/10 text-foreground/60 text-xs"
                            >
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-16">
                            <Bell size={48} className="text-foreground/30 mx-auto mb-4" />
